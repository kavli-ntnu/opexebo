...
def calc_speed(t, x, y=None, moving_average=None):
    ...
    for i, arg in enumerate((t, x, y)):
        if not isinstance(arg, (list, tuple, np.ndarray, np.ma.MaskedArray)):
            raise ValueError("{} must be an iterable, not  {}".format(arg_names[i], type(arg)))
        if not isinstance(arg, (np.ndarray, np.ma.MaskedArray)):
            arg = np.array(arg)
        if not arg.ndim == 1:
            raise ValueError("{} must be 1-dimensional, not {}-dimensional".format(arg_names[i], arg.ndim))
        if not arg.size == t.size:
            raise ValueError("{} must be the same length as `t` ({}, {})".format(arg_names[i], arg.size, t.size))

    if moving_average is not None:
        if not isinstance(moving_average, int):
            raise ValueError("`moving_average` must be None or type `int`, not {}".format(moving_average))
        if moving_average < 3 or moving_average%2 != 1:
            raise ValueError("`moving_average` must be a positive odd integer greater than 1")
