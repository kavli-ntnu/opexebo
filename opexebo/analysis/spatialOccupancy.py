...
def spatialoccupancy(positions, speed, **kwargs):
    ...
    Arguments
        ...
        positions   : ndarray. [t, x, y] or [t, x]
                        [0,:] -> timestamps
                        This matches array creation with positions=np.array([time_stamps, x, y])
        speed       : ndarray. [t, s]
    
    Keyword arguments
    
        arena_size      : float or tuple of floats. Dimensions of arena (in cm)
                            ...
                            In this function, a circle and a square are treated
                            identically.
        bin_width       : float. Bin size (in cm). Bins are always assumed square
                            default 2.0 cm.
        speed_cutoff    : float. Timestamps with instantaneous speed exceeding
                            this value are ignored. Default infinity
        sigma           : float.  Parameter for gaussian smoothing. Optional, 
                            no default. 
    '''
    ...
    # Initialise keyword arguments with defaults
    bin_size = kwargs.get('bin_size', 2.0)
    speed_cutoff = kwargs.get('speed_cutoff', np.inf)
    arena_size = kwargs.get('arena_size', 80)
    ...
    is_2d = bool(dimensionality - 2) # i.e. if positions has 2 columns, this will evaluate to False
    if type(arena_size) in (list, tuple, np.ndarray):
        if len(arena_size) == 1:
            arena_size = float(arena_size)            
        elif len(arena_size) > 2:
            raise ValueError("Keyword 'arena_size' is invalid. Provide either a float or a 2-element tuple")
    ...
    num_bins = np.ceil(arena_size / bin_size)
