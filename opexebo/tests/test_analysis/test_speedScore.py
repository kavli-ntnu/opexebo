...
def test_invalid_inputs():
    ...
    with pytest.raises(ValueError):
        ...
        tracking_times = np.arange(n)
        tracking_speeds = np.random.rand(n) * 30
        kwargs = {"bandpass":"adaptive", "lower_bound_speed":2, "upper_bound_time": 10, "speed_bandwidth": 0.01}
...
def test_random_inputs():
    ...
    tracking_times = np.arange(n)
    tracking_speeds = np.random.rand(n) * 30
    for kwargs in({"bandpass":"none"},
                  {"bandpass":"fixed", "lower_bound_speed": 2, "upper_bound_speed": 10},
                  {"bandpass":"adaptive", "lower_bound_speed":2, "upper_bound_time": 10, "speed_bandwidth": 2}):
